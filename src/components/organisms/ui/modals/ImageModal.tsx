...
const ImageModal = (props: Props) => {
  const {isVisible, setIsVisible, image, ImagePropsTextContent} = props;
  const uri = image.uri?.includes('https://')
    ? image.uri
    : MEDIA_API + image.uri;
  if (!image.width || !image.height) {
    ...
    _widthImage = image.width;
    _heightImage = image.height;
  }
